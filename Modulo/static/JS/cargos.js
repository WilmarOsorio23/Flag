document.addEventListener('DOMContentLoaded', function() {
    ...
    window.enableEdit = function() {
        let selected = document.querySelectorAll('.row-select:checked');
        if (selected.length == 0) {
            showMessage('Por favor, selecciona al menos un cargo.', 'danger');
            return false;
        }
        if (selected.length > 1) {
            showMessage('Por favor, Selecciona un solo cargo para editar.', 'danger');
            return false;
        }
    ...
    window.saveRow = function() {
        let selected = document.querySelectorAll('.row-select:checked');
        if (selected.length != 1) {
            showMessage('Error al guardar: No hay un cargo seleccionado.', 'danger');
            return;
        }
