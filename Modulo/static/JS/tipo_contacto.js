document.addEventListener('DOMContentLoaded', function() {
    ...
    window.saveRow = function() {
        ...
        let selected = document.querySelectorAll('.row-select:checked');
        if (selected.length != 1) {
            showMessage('Error al guardar: No hay un Tipo de Contacto seleccionado.', 'danger');
            return;
        }
        ...
        console.log(id)
        console.log(data)
        fetch(`/tipos_contactos/editar/${id}/`, {
            method: 'POST',
            headers: {
    ...
    window.enableEdit = function() {
        ...
        if (selected.length === 0) {
            alert('No has seleccionado ningún Tipo de Contacto para editar.');
            return false;
        }
        if (selected.length > 1) {
            alert('Solo puedes editar un Tipo de Contacto a la vez.');
            return false;
        }
