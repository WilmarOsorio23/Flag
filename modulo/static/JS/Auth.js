...
async function ocultarElementosNoPermitidos() {
  // ✅ Importante: limitar el alcance al SIDEBAR para no romper acordeones u otros .collapse de páginas
  const sidebar = document.getElementById('sidebar');
  if (!sidebar) return;

  const allFromCategories = Object.values(PERMISSION_CATEGORIES).flat();

  // ✅ Solo lee data-requires dentro del sidebar (no global)
  const domRequires = Array.from(sidebar.querySelectorAll('[data-requires]'))
  ...
  for (const [categoria, permisos] of Object.entries(PERMISSION_CATEGORIES)) {
    ...
    // Items individuales (solo sidebar)
    for (const permiso of permisos) {
      if (permisosMap[permiso] !== true) {
        sidebar.querySelectorAll(`[data-requires="${permiso}"]`).forEach(el => {
  ...
  // Elementos sueltos con data-requires (solo sidebar)
  sidebar.querySelectorAll('[data-requires]').forEach(el => {
  ...
  // ✅ Submenús vacíos: SOLO los UL del sidebar (NO acordeones)
  // Antes: document.querySelectorAll('.collapse') -> rompía acordeones.
  sidebar.querySelectorAll('ul.collapse.list-unstyled').forEach(submenu => {
